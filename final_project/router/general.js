...
public_users.post("/register", async (req, res) => {
  ...
  if (username && password) {
    if (isValid(username)) { 
      ...
      return res.status(404).send(JSON.stringify({message: "User already exists!"}, null, 4));    
    }
  } 
  return res.status(404).send(JSON.stringify({message: "Unable to register user (missing username or password)."}, null, 4));
});
...
// 3. Get book details based on ISBN using Promises/Async-Await
...
// 4. Get book details based on author using Promises/Async-Await
public_users.get('/author/:author', async function (req, res) {
  const author = req.params.author;

  try {
    const fetchBooksByAuthor = (authorName) => new Promise((resolve, reject) => {
      let matchingBooks = {};
...
// 5. Get all books based on title using Promises/Async-Await
public_users.get('/title/:title', async function (req, res) {
  const title = req.params.title;

  try {
    const fetchBooksByTitle = (titleName) => new Promise((resolve, reject) => {
      let matchingBooks = {};
      Object.keys(books).forEach((key) => {
...
// 6. Get book review using Promises/Async-Await
public_users.get('/review/:isbn', async function (req, res) {
  const isbn = req.params.isbn;

  try {
    const fetchReviews = (id) => new Promise((resolve, reject) => {
      if (books[id]) {
